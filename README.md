...
> **Live agent integration status: VERIFIED (2026-09-17, `gpt-5-mini`).** Live traces show the model requesting MCP tools and recovering from an injected tool failure. See [Section 6](#6-agent-traces-live-integration-evidence).
...
## 3. Architecture and MCP call flow
...
reviewer.py ──► mcp_client.py  ── mcp.Client(StdioServerParameters) ──►  mcp_server.py (subprocess)
   │               list_tools() / call_tool()   JSON-RPC over stdio        ├─ get_product     → data/products.json
   │               logs: tool, args, ms, ok, source IDs                     ├─ search_policy   → TF-IDF over data/policies.json
   │                                                                        └─ check_campaign  → checks.py (regex rules)
   ...
   │        loop (≤ CG_MAX_TOOL_ROUNDS): model → function_call → MCP call_tool → function_call_output
   ...
   └─ F. report_validation, then status.decide_review_status(required_checks, findings) → ReviewReport
```
...
- **Final status is decided deterministically in Python** by a single function, `status.decide_review_status`. Model output (including the model's summary) never sets the status. The report records five **required checks**, each `complete`, `incomplete`, or `not_run`:
  ...
  Precedence is fixed:
  1. **Any required check not complete** → `needs_review`. This takes precedence over findings.
  2. **Otherwise, any validated finding** → `issues_found`.
  3. **Only a complete review with no findings** → `no_issues_detected`.
...
## 4. Sample review (deterministic_only, campaign C30)
...
- **Prompt and policy tuning used dev only, and the tuning history is recorded.**
  ...
  3. I clarified the fictional POL-008 text (dataset → `cg-synthetic-v1.1`) and added prompt rule 8 (prompt `v2`). Dev then scored P 1.00 / R 1.00 (`results/eval_dev_prompt_v2.json`). A perfect score on 10 examples is likely overfit.
...
## 6. Agent traces (live integration evidence)
